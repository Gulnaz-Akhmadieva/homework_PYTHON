...
db_connection = 'postgresql://postgres:123@localhost:5432/postgres'
...
def test_add_student():
    with create_engine(db_connection).connect() as conn:
        ...
        row = conn.execute(text("select level from student where level = 'Test Level'")).fetchone()
        assert row.level == "Test Level"
